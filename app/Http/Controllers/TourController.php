...
class TourController extends Controller
{
    ...
    public function foodtour(Request $request)
    {
        ...
        $destination = $request['destination'];
        $tour_result = collect();
        $tours = Tour::where('is_outbound', '<>', 1)->where('tour_type', 3)->get(); //long tour =1
        ...
        $paginatedSearchResults->appends(['destination' => $request['destination']]);
        $paginatedSearchResults->appends(['tour_type' => $request['tour_type']]);
        $paginatedSearchResults->setPath(route('frontend.tour.domestic.foodtour'));
        $locations = Location::where('is_domestic', 1)->get();

        return view('frontend.pages.tour.foodtour', ['locations' => $locations, 'list_tour' => $paginatedSearchResults]);
